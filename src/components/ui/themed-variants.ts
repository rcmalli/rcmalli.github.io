...
export const themedLink = cva('transition-colors', {
  variants: {
    variant: {
      ...
      text: 'text-foreground/70 underline-offset-4 hover:text-[color:var(--accent-blue)] hover:underline hover:decoration-[color:var(--accent-blue)]',
      toc:
  ...
  compoundVariants: [{ variant: 'toc', active: true, class: 'text-[color:var(--accent-blue)]' }],
  defaultVariants: { variant: 'text', active: false },
});

export const themedIconButton = cva(
  'group relative inline-flex items-center justify-center rounded-sm transition-all overflow-hidden ' +
    "before:content-[''] before:absolute before:inset-0 before:rounded-sm before:transition-opacity",
  {
    variants: {
      tone: {
        accent:
          'text-foreground/60 hover:text-[color:var(--accent-blue)] ' +
          'before:bg-[color:var(--accent-blue)] before:opacity-0 hover:before:opacity-10',
        neutral:
          'text-muted-foreground hover:text-foreground ' +
          'before:bg-foreground before:opacity-0 hover:before:opacity-[0.08]',
        nav:
          'text-foreground/60 hover:text-[color:var(--accent-blue)] ' +
          'before:bg-foreground before:opacity-0 hover:before:opacity-10',
      },
      size: {
        default: 'p-2.5 duration-300',
        sm: 'p-2 duration-200',
      },
    },
...
export type ThemedIconButtonVariants = VariantProps<typeof themedIconButton>;
